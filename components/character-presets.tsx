...
export default function CharacterPresets({ presets, onPresetAdd, addedPresets }: CharacterPresetsProps) {
      ...
      {presets.map((preset) => (
        <button
          ...
          className={`relative group transition-all duration-500 transform ${
            visiblePresets.includes(preset.id) ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
          }`}
        >
          <div
          ...
          >
            <div className="text-sm font-medium text-fg-primary mb-1">{preset.label}</div>
            <div className="text-xs text-fg-muted flex items-center gap-1">
              <span>{preset.sizeHint} 字符</span>
              <span className="text-[10px] uppercase tracking-widest text-fg-muted/70">
                {preset.type === 'inline' ? 'INLINE' : 'REMOTE'}
              </span>
            </div>
          </div>
        </button>
